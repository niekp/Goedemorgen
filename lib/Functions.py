...
class Functions:
    # pylint: disable=no-self-argument, no-member, unsupported-membership-test, unsubscriptable-object
    def html2plain(text):
    ...
    def plain2hml(text):
        ...
        text = text.replace("- [ ]", "<input type='checkbox' />")
        text = text.replace(
            "- [x]", "<input type='checkbox' checked='checked' />")
        return text
    ...
    def getDatabase(config_full, database_name, create_query):
        ...
        filename = "{0}/{1}.db".format(
            config_full["Runtime"]["userdir"], database_name)
